...
Le destructeur d'une classe est une méthode spéciale appelée `__del__`. Cette
méthode est appelée lorsque l'instance de la classe est détruite. C'est à dire
lorsque l'objet n'est plus référencé et que le ramasse-miettes de Python décide
de libérer la mémoire occupée par l'objet.

Le destructeur est utilisé pour libérer les ressources utilisées par l'instance
de la classe. Les ressources peuvent être des fichiers ouverts, des connexions
réseau, ou d'autres ressources système. Le destructeur peut également être
utilisé pour effectuer des actions de nettoyage avant que l'objet ne soit
détruit, comme enregistrer des données ou afficher un message.

Cependant, il est important de noter que le destructeur n'est pas
garanti d'être appelé dans tous les cas, notamment lorsque le programme se
termine ou lorsqu'il y a des références circulaires. Par conséquent, il est
généralement recommandé d'utiliser des gestionnaires de contexte ou des blocs
`try...finally` pour gérer les ressources de manière plus fiable.
...
class Point:
    ...
    def __del__(self):
        print(f"Le point ({self.x}, {self.y}) a été détruit.")
```
