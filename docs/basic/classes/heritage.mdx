...
En Python, on hérite d'une classe en plaçant la classe mère entre parenthèses
après le nom de la classe fille :

```python
class CompteEpargne(CompteBancaire):   # CompteEpargne hérite de CompteBancaire
    pass
```

La classe fille **reçoit automatiquement** tous les attributs et méthodes de la
classe mère. Un `CompteEpargne` peut donc déjà déposer, retirer et s'afficher
sans rien réécrire.

### Ajouter des attributs et des méthodes

La classe fille peut définir ses propres attributs et méthodes **en plus** de
ceux hérités. Pour initialiser les attributs de la classe mère, on appelle
`super().__init__()` (nous y reviendrons en détail [plus bas](#la-méthode-super)) :

```python
class CompteEpargne(CompteBancaire):
    def __init__(self, titulaire, solde=0.0, taux=0.02):
        super().__init__(titulaire, solde)  # initialise la partie CompteBancaire
        self.taux = taux                   # nouvel attribut propre à CompteEpargne

    def appliquer_interets(self):          # nouvelle méthode propre à CompteEpargne
...
Une classe fille peut aussi **remplacer** une méthode héritée en la redéfinissant
avec le même nom. Voici `CompteCredit`, où le découvert est autorisé : la méthode
`retirer` doit donc se comporter différemment :
...
class CompteCredit(CompteBancaire):
    ...
    def retirer(self, montant):             # redéfinition de CompteBancaire.retirer
        if montant > self._solde + self.limite_credit:
            raise ValueError("Limite de crédit dépassée.")
        self._solde -= montant              # le solde peut devenir négatif

credit = CompteCredit("Bob", limite_credit=500)
credit.retirer(300)   # autorisé — découvert dans la limite
print(credit)         # Compte(Bob, -300.00 $)
```

Voici un résumé de la hiérarchie obtenue :
