...
<Aside type="note">Les noms de fonctions `foo`, `bar`, `baz` sont des noms de
...
clairement ce que fait la fonction.</Aside>
...
<Aside type="tip">Comme dans le code `for _ in range(fois):` ci-haut, vous pouvez 
...
<Aside type="danger">Ne jamais utiliser un objet mutable comme valeur par
...
[décorateurs](/programmation-python/concepts-avancés/concepts_avances/) utilisent cette
...
<Aside type="tip">À moins d'avoir une très bonne raison, mieux vaut utiliser
une liste comme paramètre plutôt qu'un nombre variable de paramètres.</Aside>
...
<Aside type="caution">À moins d'avoir une bonne raison, mieux vaut éviter
d'utiliser `*` et `**` pour passer des paramètres à une fonction. Cela rend le
code plus difficile à lire et à comprendre.</Aside>